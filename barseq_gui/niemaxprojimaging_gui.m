function niemaxprojimaging_gui(app, cyclename, chseq, tformch, pausetime, posinfo, tform, srcpath, destpath)
...
%%
app.MessageTextArea.Value= ['cyclename=', cyclename , ' srcpath= ', srcpath ' localpath= ', destpath  ];
...
destpath=fullfile(destpath,cyclename);
app.MessageTextArea.Value=[app.MessageTextArea.Value(:)', {sprintf('Creating local designated maxproj folder.')}];
...
while nn<5
    ...
    parfor (n=1:length(newpos),max_worker)
        ...
        for m=1:length(ch)
            ...
            if ismember(chseq(m),tformch)&&~isempty(tform)
                ...
                if m==1
                    imwrite(im,fullfile(destpath,['MAX_',posname,'.tif']));
                else
                    imwrite(im,fullfile(destpath,['MAX_',posname,'.tif']),'WriteMode','Append');
                end
            else
                if m==1
                    imwrite(uint16(max(im,[],3)),fullfile(destpath,['MAX_',posname,'.tif']));
                else
                    imwrite(uint16(max(im,[],3)),fullfile(destpath,['MAX_',posname,'.tif']),'WriteMode','Append');
                end
            end
            %toc
        ...
        %toc
        %%
        newfiles{n}=fullfile(destpath,['MAX_',posname,'.tif ']);
    ...
    %         eval(['!start /B scp ',fnameassemble,remotelogin,':',remotepath,'/',cyclename,'/']);
    %     end
    passedtime=toc;
end
cd ..
